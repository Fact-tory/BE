...
import com.commonground.be.global.infrastructure.encryption.FieldEncryption;
...
public class AdminTokenGenerator implements CommandLineRunner {
	...
	public void generateAndLogAdminToken() {
		try {
			...
			log.info("curl 예시:");
			log.info(
					"  curl -H \"Authorization: Bearer {}\" http://localhost:8080/api/v1/admin/users",
					adminToken);
			log.info("  curl -H \"X-Admin-Token: {}\" http://localhost:8080/api/v1/users/profile",
					adminToken);
			log.info("=".repeat(80));
